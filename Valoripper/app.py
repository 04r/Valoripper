...
def load_skin_map():
    """Load cached Riot skin names directly from the local content_cache.json"""
    global SKIN_MAP
    skin_data = valapi.load_cached('weapons')  # Use the new skin data cache
...
ttk.Label(root, text="Valoripper", style="Header.TLabel").pack(anchor="w", padx=10, pady=(10, 5))
match_label = ttk.Label(root, text="Match: ..."); match_label.pack(anchor="w", padx=10)
map_label = ttk.Label(root, text="Map: ..."); map_label.pack(anchor="w", padx=10)
server_label = ttk.Label(root, text="Server: ..."); server_label.pack(anchor="w", padx=10, pady=(0, 10))
...
ttk.Label(left_frame, text="Players").pack(anchor="w")
players_list = tk.Listbox(left_frame, bg="#1e1e1e", fg="white", selectbackground="#333333", relief="flat")
players_list.pack(fill="both", expand=True, padx=(0, 5))

right_frame = ttk.Frame(main_frame)
right_frame.pack(side="left", fill="both", expand=True)
ttk.Label(right_frame, text="Loadout").pack(anchor="w")
skins_list = tk.Listbox(right_frame, bg="#1e1e1e", fg="white", selectbackground="#333333", relief="flat")
...
def refresh_data():
    ...
    try:
        ...
        for p in current_players:
            players_list.insert(tk.END, p.ign.username)
    except Exception as e:
...
def on_player_select(event):
    ...
    skins_list.insert(tk.END, f"Fetching {player.ign.username}'s loadout...")
    def worker():
        loadout = get_player_loadout(current_match_id, puuid)
        skins_list.delete(0, tk.END)
        for item in loadout:
            skins_list.insert(tk.END, f"• {item}")
